...
export function ProtectedFilesPage() {
  ...
  const fileContentQuery = useQuery({
    ...
    queryFn: () =>
      protectedService.getFileContent(selectedFile!.id, Number(localDeviceId)),
    enabled: Boolean(
      selectedFile?.id && localDeviceId !== null && selectedFile.type === "pdf",
    ),
  });
  ...
  return (
    <PageShell
      description="This is for demo of a full Zero-Trust access chain: authentication, MFA, RBAC, and device compliance."
    ...
    >
      ...
      <section className="ui-panel">
        ...
        <p className="mt-2 text-zinc-600">
          This page uses the local ZT Agent to detect the current device. The
          backend still verifies MFA, role, ownership, and device compliance
        ...
        <div className="mt-5 rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
          ...
          {localAgentQuery.isLoading && (
            <div className="mt-3">
              <LoadingState message="Detecting local ZT Agent..." />
            </div>
          )}
          ...
          {deviceQuery.isLoading && (
            <div className="mt-3">
              <LoadingState message="Loading current device posture..." />
            </div>
          )}
          {deviceQuery.isError && (
            <div className="mt-3">
              <ErrorState message={getApiErrorMessage(deviceQuery.error)} />
            </div>
          )}
          {deviceQuery.data && (
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
              <div>
                <strong className="text-zinc-950">
                  #{deviceQuery.data.id} {deviceQuery.data.hostname}
                </strong>
                <p className="mt-1 text-sm text-zinc-600">
                  {deviceQuery.data.os_type}
                  {deviceQuery.data.os_version
                    ? ` ${deviceQuery.data.os_version}`
                    : ""}
                </p>
              </div>
              <StatusBadge
                tone={deviceQuery.data.is_compliant ? "success" : "danger"}
              >
        ...
        {filesMutation.isError && (
          <div className="mt-4">
            <ErrorState message={getApiErrorMessage(filesMutation.error)} />
          </div>
        )}
      </section>
        ...
        <section className="grid gap-4">
          <div className="rounded-2xl border border-green-700/15 bg-green-50 p-5 text-green-800">
            ...
            <p className="mt-1 text-green-800/80">
              User {filesMutation.data.user} accessed files from device{" "}
              {filesMutation.data.device}.
            </p>
          </div>
                ...
                <tbody>
                  {filesMutation.data.files.map((file) => (
                    ...
                    >
                      ...
                      <td className={tdClassName}>
                        <StatusBadge tone={sensitivityTone(file.sensitivity)}>
                          {file.sensitivity}
                        </StatusBadge>
                      </td>
                    </tr>
              ...
              {selectedFile ? (
                <>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <StatusBadge
                      tone={sensitivityTone(selectedFile.sensitivity)}
                    >
                  ...
                  {selectedFile.type === "pdf" && (
                    <div className="mt-5 overflow-hidden rounded-2xl border border-zinc-950/10 bg-zinc-100">
                      {fileContentQuery.isLoading && (
                        <LoadingState message="Loading PDF preview..." />
                      )}
                      {fileContentQuery.isError && (
                        <ErrorState
                          message={getApiErrorMessage(fileContentQuery.error)}
                        />
                      )}
                      {pdfUrl && (
                        <iframe
                    ...
                    <div className="mt-5 max-h-[520px] overflow-auto rounded-2xl border border-zinc-950/10">
                      <table className="min-w-full border-collapse text-left text-xs">
                        <thead className="sticky top-0 bg-zinc-100">
                          <tr>
                            {parseCsv(selectedFile.content)[0]?.map(
                              (header) => (
                                <th
                                  className="border-b border-zinc-950/10 px-3 py-2 font-semibold text-zinc-700"
                                  key={header}
                                >
                                  {header}
                                </th>
                            ...
                            .map((row, rowIndex) => (
                              <tr key={`${selectedFile.name}-${rowIndex}`}>
                                {row.map((cell, cellIndex) => (
                                  <td
                                    className="border-b border-zinc-950/5 px-3 py-2 text-zinc-700"
                                    key={cellIndex}
                                  >
                                    {cell}
                                  </td>
                ...
                <p className="text-sm text-zinc-600">
                  Select a file to preview its protected contents.
                </p>
              )}
            </article>
          </section>
